...
public class ManifestBuilder extends DefaultManifestBuilder {
	...
	public Manifest build(Manifest original) {
		...
		String newiPojo=IPojoPurify.purify(ipojoComponents);
//		attrs.putValue("iPOJO-Components", newiPojo);
//		System.out.println("type=:"+attrs.getValue("iPOJO-Components").getClass());
		return ret;
	}
	...
	public void checkIgnoreImport(Manifest ret) {
		...
		String importIgnoreZP = (String) attrs.getValue("Import-Ignore-ZP");

		String imports = (String) attrs.getValue("Import-Package");
		System.out.println("imports==" + imports + "::importIgnore=" + importIgnore+"::importIgnoreZP:"+importIgnoreZP);
		if(importIgnore==null){
			importIgnore = importIgnoreZP;
		}else
		{
			importIgnore = importIgnoreZP+","+importIgnore;
		}
		if (importIgnore != null) {
			String orgimport[] = imports.split(", ");
			for (String ignore : importIgnore.split(",")) {
				for (int i = 0; i < orgimport.length; i++) {
					if (ignore!=null&&ignore.length()>0&&orgimport[i] != null && orgimport[i].trim().startsWith(ignore)) {
						orgimport[i] = null;
					}
				}
			}
